  ...
  [].forEach.call(goods, function (item, i, arr) {
    ...
    item.activateSelectedStyle = function () {
      if (this.getAmount()) {
        ...
        this.querySelector(".goods__parameter--amount").previousElementSibling.style.borderRightColor = "#b5daee";
      ...
      [].forEach.call(uploadedFiles, function (uploadedFile, i, arr) {
        switch (uploadedFile.name.split(".")[uploadedFile.name.split(".").length - 1]) {
          ...
          case "jpg":
            ...
            passport.className = "form__icon form__icon--passport";
            fileLabel.appendChild(passport);
            if (uploadedFile.name.length > MAX_FILE_NAME_CHARACTERS) {
              passport.innerHTML = uploadedFile.name.slice(0, MAX_FILE_NAME_CHARACTERS) + "...";
            } else {
              passport.innerHTML = uploadedFile.name;
            }
            break;
          case "pdf":
          case "doc":
          case "docx":
            ...
            resume.className = "form__icon form__icon--resume";
            fileLabel.appendChild(resume);
            if (uploadedFile.name.length > MAX_FILE_NAME_CHARACTERS) {
              resume.innerHTML = uploadedFile.name.slice(0, MAX_FILE_NAME_CHARACTERS) + "...";
            } else {
              resume.innerHTML = uploadedFile.name;
            }
        }
        if (countOfUploadedFiles < MAX_FILES_COUNT) {
